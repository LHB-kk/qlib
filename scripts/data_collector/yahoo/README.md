...
### Get Qlib data(`bin file`)
  ...
  - parameters:
    - `target_dir`: save dir, by default */root/qlib_data/cn_data*
  ...
  - examples:
    ...
    # cn 1min
    python scripts/get_data.py qlib_data --target_dir /root/qlib_data/cn_data_1min --region cn --interval 1min
    # us 1d
...
> If the above ready-made data can't meet users' requirements,  users can follow this section to crawl the latest data and convert it to qlib-data.
  ...
  2. normalize data: `python scripts/data_collector/yahoo/collector.py normalize_data`
     ...
     - parameters:
      ...
      - examples:
        ...
        # normalize 1min cn
        python collector.py normalize_data --qlib_data_1d_dir /root/qlib_data/cn_data --source_dir ~/.qlib/stock_data/source/cn_data_1min --normalize_dir ~/.qlib/stock_data/source/cn_1min_nor --region CN --interval 1min
  ...
  3. dump data: `python scripts/dump_bin.py dump_all`
     ...
     - examples:
       ```bash
       # dump 1d cn
       python dump_bin.py dump_all --data_path ~/.qlib/stock_data/source/cn_1d_nor --qlib_dir /root/qlib_data/cn_data --freq day --exclude_fields date,symbol --file_suffix .csv
       # dump 1min cn
       python dump_bin.py dump_all --data_path ~/.qlib/stock_data/source/cn_1min_nor --qlib_dir /root/qlib_data/cn_data_1min --freq 1min --exclude_fields date,symbol --file_suffix .csv
       ```
...
## Using qlib data
  ...
  qlib.init(provider_uri="/root/qlib_data/cn_data", region="cn")
  df = D.features(D.instruments("all"), ["$close"], freq="day")

  # 1min data cn
  # freq=1min
  qlib.init(provider_uri="/root/qlib_data/cn_data_1min", region="cn")
